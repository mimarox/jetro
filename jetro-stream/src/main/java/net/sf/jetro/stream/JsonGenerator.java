...
import static net.sf.jetro.stream.JsonScope.EMPTY_ARRAY;
import static net.sf.jetro.stream.JsonScope.EMPTY_DOCUMENT;
import static net.sf.jetro.stream.JsonScope.EMPTY_OBJECT;
import static net.sf.jetro.stream.JsonScope.NONEMPTY_ARRAY;
import static net.sf.jetro.stream.JsonScope.NONEMPTY_DOCUMENT;
import static net.sf.jetro.stream.JsonScope.NONEMPTY_OBJECT;
 ...
 * @since 1.0
 */
public class JsonGenerator implements Closeable, Flushable {
	 ...
	 */
	public JsonGenerator(Appendable out) {
		if (out == null) {
	 ...
	 * bracket.
	 */
	private JsonGenerator open(int empty, String openBracket) throws IOException {
		beforeValue(true);
		push(empty);
	 ...
	 * given bracket.
	 */
	private JsonGenerator close(int empty, int nonempty, String closeBracket) throws IOException {
		int context = peek();
	 ...
	 * @return this writer.
	 */
	public JsonGenerator value(Number value) throws IOException {
		if (value == null) {
			return nullValue();
		}
	...
	private void beforeName() throws IOException {
		...
		if (context == NONEMPTY_OBJECT) { // first in object
			out.append(',');
		} else if (context != EMPTY_OBJECT && !lenient) { // not in an object!
			throw new IllegalStateException("Nesting problem.");
		}
		newline();
